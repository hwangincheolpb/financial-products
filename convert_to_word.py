...
def add_text_box(lines):
    """텍스트 박스 추가"""
    for line in lines:
        p = doc.add_paragraph(line, style='List Bullet')
        p.paragraph_format.left_indent = Inches(0.2)

def add_image(image_path, width=6.0):
    """이미지 추가"""
    # 절대경로로 변환
    ...
    if os.path.exists(full_path):
        ...
        doc.add_paragraph(f"[이미지 없음: {full_path}]")
...
doc.add_paragraph("타임폴리오 The Time (Long-Short)", style='Heading 3')
add_text_box([
...
doc.add_paragraph("디에스 Brave S (Long-Biased)", style='Heading 3')
add_text_box([
...
# 문서 저장
output_path = "c:\\dev\\active-projects\\fund-comparison-web\\사모펀드_투자제안서.docx"
